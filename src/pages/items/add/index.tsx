...
import CustomSelectUi from "@/components/CustomSelect";
...
const ItemForm: FC = () => {
  ...
  const { addRolesObject: addItemObject } = useSelector(
    (state: any) => state.itemReducer
  );
  ...
  }, [chips, quantity, minLevel, price, notes, threshold]);
        ...
        <div className="w-56">
          <SelectUi
            ...
            onHandleChange={(newVal) => {
              dispatch(
                addItemSetter({ ...addItemObject, folder: Number(newVal) })
              );
              ...
              <div className={`relative ${field.width}`} key={index}>
                ...
                {field.variant === "select" && (
                  <CustomSelectUi
                    ...
                    onHandleChange={(option: any) => {
                      field.setterFn(option.id);
                    }}
                  />
                )}
              </div>
